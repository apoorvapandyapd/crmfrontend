...
const TBL_SHOW_RECORDS = process.env.TBL_SHOW_RECORDS === undefined ? 10 : process.env.TBL_SHOW_RECORDS;
const TBL_PER_PAGE = process.env.TBL_PER_PAGE === undefined ? 2 : process.env.TBL_PER_PAGE;
const Ticket = () => {
    ...
    function fetchData(){
        CustomRequest('list-ticket', {}, client.token, (res)=> {
            if(res?.error) {
            ...
            <Fragment>
                    ...
                    </div>
                    {
                        currentRecords.map(value=>(
                            <div className="ticket-list-wrapper" key={value.id}>
