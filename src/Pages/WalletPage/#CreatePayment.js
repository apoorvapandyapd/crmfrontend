                        ...
                        <div className="box-wrapper w-700">
                            <div className="card-body create-ticket p-0 bg-white">
                                <h2 className="mb-0 px-40">
                                    <Link to='/list/wallet'><a href="#" className="back-arrow">
                                ...
                                <div className='p-40'>
                                        ...
                                        {data !== null && data.map((method, i) =>
                                            <>
                                                <div className="form-group col-6 col-sm-3">
                                                    <FormControl type="radio" value={method.id}
                                                                 data-name={method.payment_name}
                                                                 onChange={(e) => handlePayment(e)}
                                                                 name="account_group_id" id={`${method.id}-${i}`}/>
                                                    <label htmlFor={`${method.id}-${i}`}><img src={method.logo} alt=""/>
                                                        <p className='text-center mt-2 pb-2'>{method.payment_name}</p>
                                                    </label>
                                    ...
                                    {paymentName == 'Stripe' || paymentName == 'Paypal' ? <>
                                        <form onSubmit={submitPaymentGroup} id='paymentForm'><Form.Group
                                            className="mt-4">Coming soon please contact on this
                                            xyz@parkmoney.com</Form.Group></form>
                                    </> : <>
                                        <form onSubmit={submitPaymentGroup} id='paymentForm'>
                                                        ...
                                                        fieldData !== null && fieldData.map(val => (
                                                            <Col md={6} className="mt-3">
                                                                {val.type != 'textarea' ? <><label>{val.label}</label>
                                                                        <input type={val.type} className='form-control'
                                                                               name={val.key} id={val.key}
                                                                               placeholder={val.label}
                                                                               onChange={handleInput}/></>
                                                                    : <><label>{val.label}</label><FormControl
                                                                        maxLength={100} as="textarea" name={val.key}
                                                                        id={val.key} onChange={handleInput}
                                                                        placeholder={val.label} rows={2}/></>}
                                            ...
                                            <div
                                                ...
                                                <button type="submit"
                                                        className="btn btn-primary float-end btn btn-primary mb-3 mb-sm-0 order-1 order-sm-0">Create
                                                </button>
                                            </div>
                                        </form>
                                    </>}
