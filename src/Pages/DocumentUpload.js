...
const DocumentUpload = (props) => {
    ...
    async function fetchData() {
        ...
        CustomRequest('list-namedocument', data, client.token, (res) => {
            if (res?.error) {
    ...
    const handleInput=async(e)=>{
        ...
        await documentData.map((val) => {
            if (val.id === parseInt(e.target.value)) {
                ...
                CustomRequest('get-document', data, client.token, (res) => {
                    if (res?.error) {
                        console.log(res.error);
                        if (res?.error?.response.status === 401) {
                            dispatch(redirectAsync());
                        ...
                        if (res.data.status_code === 200) {
                            setSelectDocumentId(res.data.data.id);
                            setSelectedDocument(res.data.data);
                            setSelectedDocumentStatus(res.data.data.status);
                        }
    ...
    const uploadDocument=(e)=>{
        ...
        data.append(docName, imageData.image);

        if (imageData.image === null) {
            data.append('selected_id', selectDocumentId);
        }
        if (imageData.password !== null) {
            data.append('password', imageData.password);
        }

        CustomRequest('store-document', data, { token: client.token, content_type: 'multipart/form-data' }, (res) => {
            if (res?.error) {
                console.log(res.error);
                setLoading(false);
                if (res?.error?.response.status === 401) {
                ...
                if (res.data.status_code === 200) {
                    ...
                    setPwdDocState(res.data.data.uploadedDocuments.map((data, i) => (false)));
                    fetchData();
                    dispatch(updateClientDataAsync(res.data.data.clientDetails, client.token));
