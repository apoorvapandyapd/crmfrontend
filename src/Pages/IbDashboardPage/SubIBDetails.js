...
function SubIBDetails(props) {
    allSubIbs = props.data.data.sub_ibs.data;
    // let propClientData = props.data.data.sub_clients.data;
    const [clientData, setClientData] = useState(allSubIbs)
        ...
        if (search.trim() !== '' || selclient) {
            ...
            if (search.trim() !== '') {
                filteredAcc = filteredAcc.filter((c) => {
                    ...
                    if (first_namesearch || statussearch || emailsearch || phonesearch) {
                        return c;
                    }
                });
            }
