...
const Changepassword = () => {
    ...
    function fetchData(data) {
        ...
        CustomRequest('reset-password', formData, client.token, (res)=> {
            if(res?.error) {
                                    ...
                                    <div className="w-100 border-0">
                                        ...
                                        <form onSubmit={changepasswordSubmitHandler}>
                                            <Form.Group className="mb-3  position-relative">
                                                <Form.Control type={oldpassstate === true ? 'text' : 'password'}
                                                              name="old_password" placeholder="Old Password"/>
                                                <span className="password-icon"
                                                      onClick={handleOldPass}>{oldpassstate === false ?
                                            ...
                                            <Form.Group className="mb-3 position-relative">
                                                <Form.Control type={newpassstate === true ? 'text' : 'password'}
                                                              name="password" placeholder="New Password"/>
                                                <span className="password-icon"
                                                      onClick={handleNewPass}>{newpassstate === false ?
                                                        <VisibilityIcon width="16" height="16" /> :
                                                        <VisibilityOffIcon width="16" height="16" />
                                                }</span>
                                            ...
                                            <Form.Group className="mb-3 position-relative">
                                                <Form.Control type={confirmpassstate === true ? 'text' : 'password'}
                                                              name="confirm_password" placeholder="Confirm Password"/>
                                                <span className="password-icon"
                                                      onClick={handleConfirmPass}>{confirmpassstate === false ?
                                                        <VisibilityIcon width="16" height="16" /> :
                                                        <VisibilityOffIcon width="16" height="16" />
                                                }</span>
                                                <small className="text-danger">{error.confirm_password}</small>
                                            </Form.Group>
                                            <Button type="submit" className="btn btn-primary float-end">Change
                                                Password</Button>
                                        </form>
                                    </div>
                                </div>
