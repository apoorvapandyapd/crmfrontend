...
function IbAllClientWithdraw() {
    ...
    const [ibWithdraw, setIbWithdraw] = useState(null);
    ...
    let csvdata = Array.isArray(ibWithdraw) && ibWithdraw.map(data => {
        return {
                        ...
                        <select name='selclient' value={filter.selclient} className="form-control select mx-sm-3 mx-0 my-2 my-sm-0" onChange={clientChangeHandler}>
                            ...
                            {clientsArr.map((cl, i) =>
                                <option key={i} value={cl.clientid}>{cl.first_name} {cl.last_name}</option>
                                    ...
                                    <Link to="#" onClick={fetchAllWithdraw} className="me-2">
                                        <RefatchIcon width="24" height="24" />
                                    </Link>
                                    <CSVLink
                                        data={csvdata}
                                        filename={"clientwithdrawals.csv"}
                                        target="_blank"
                                    >
            ...
            <div className="fix-table-height mt-3">
                <div className="table-responsive">
                    <table className="table m-0">
                        ...
                        <tbody>
                            {Array.isArray(ibWithdraw) && ibWithdraw.length === 0 ? <tr className='text-center'><td colSpan="7">No records found</td></tr> : ibWithdraw.map((acc, id) =>
                            <tr key={id}>
...
export default IbAllClientWithdraw;